...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SDIoU=False, eps=1e-7, delta=0.5):  # SDIoU change
    """
    ...
    Args:
        ...
        SDIoU (bool, optional): If True, calculate Scale-based Dynamic IoU. Defaults to False.  # SDIoU change
        eps (float, optional): A small value to avoid division by zero. Defaults to 1e-7.
        delta (float, optional): SDIoU dynamic scale coefficient upper bound. Defaults to 0.5.  # SDIoU change

    Returns:
        (torch.Tensor): IoU, GIoU, DIoU, CIoU, or SDIoU values depending on the specified flags.
    """
    ...
    # IoU
    iou = inter / union
    # SDIoU change: SDIoU reuses the CIoU geometry terms and adds a dynamic scale coefficient.
    if CIoU or DIoU or GIoU or SDIoU:
        ...
        if CIoU or DIoU or SDIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU or SDIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                # SDIoU change: migrate the reference dynamic scale term for detection boxes.
                if SDIoU:
                    beta = ((w2 * h2 * delta) / 81).clamp(max=delta)
...
class DetMetrics(SimpleClass):
    ...
    def keys(self):
        ...
        return [
            ...
            "metrics/mAP50(B)",
            "metrics/mAP50-95(B)",
            "metrics/false_alarm_rate(B)",
        ]

    def mean_results(self):
        """Calculate mean detection metrics and false alarms per image."""
        return self.box.mean_results() + [self.false_alarm_rate]

    def class_result(self, i):
        """Return the result of evaluating the performance of an object detection model on a specific class."""
        return self.box.class_result(i) + (self.false_alarm_rate,)
